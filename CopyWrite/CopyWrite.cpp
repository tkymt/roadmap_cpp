// CopyWrite.cpp : このファイルには 'main' 関数が含まれています。プログラム実行の開始と終了がそこで行われます。
//
...
public:
    ...
    MyString(const MyString& other) :data(other.data) {
        std::cout << "Copied using the Copy-Wite ideom." << std::endl;
    }

    // Make a copy only if we want to modify the data.
    void write(const std::string& str) {
        // Check if ther's more than one reference.
        if (!data.unique()) {
